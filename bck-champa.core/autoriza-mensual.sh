...
ruta_s3="s3://$bucket/autoriza/mensual/$fecha_actual"

# Verificar si la carpeta ya existe en S3
if aws s3api head-object --bucket "$bucket" --key "autoriza/mensual/$fecha_actual/" 2>/dev/null; then
    # Si existe, mostrar un mensaje
    echo "Ya existe la carpeta"
else
    # Si no existe, crear la carpeta y mostrar un mensaje de error si falla
    if ! aws s3api put-object --bucket "$bucket" --key "autoriza/mensual/$fecha_actual/" 2>> /home/srvbackup/errores/autoriza/"$nombre_archivo"; then
