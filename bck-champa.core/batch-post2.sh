...
directorios_local=("/batch/historicos" "/batch/home/bctrl-m" "/batch/usr/cobol/salto/resumenes")
...
sincronizar_directorio() {
    local directorio="$1"
    aws s3 sync "$directorio" "$ruta_s3""$directorio"  --exclude "/batch/historicos/inv*" --exclude "/batch/historicos/histo*" --exclude "/batch/historicos/conversion" --exclude "/batch/historicos/acu"  --only-show-errors 2>> /home/srvbackup/errores/batch/"$nombre_archivo"
}
