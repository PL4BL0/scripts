...
echo "$fecha_inicial" > /home/srvbackup/errores/batch/"$nombre_archivo"
...
directorios_local=("/batch/disk4" "/batch/home" "/batch/tn" "/batch/historicos" "/batch/tn/galicia" "/batch/disk2" "/batch/conversion"
 "/batch/dbcore" "/batch/procesos" "/batch/home/bctrl-m" "/batch/usr/cobol" "/batch/usr/cobol/pdr" "/batch/usr/cobol/salto/liq"
 "/batch/usr/cobol/salto/vigia" "/batch/usr/cobol/salto/listados" "/batch/usr/cobol/tmp" "/batch/usr/cobol/salto/historico" "/batch/usr/cobol/salto/mensual")

# Nombre del bucket S3
bucket="backup-core-nx"

# Ruta en S3 donde quieres sincronizar los archivos
ruta_s3="s3://$bucket/batch/diario/$fecha_actual"

# Verificar si la carpeta ya existe en S3
if aws s3api head-object --bucket "$bucket" --key "batch/diario/$fecha_actual/" 2>/dev/null; then
    # Si existe, mostrar un mensaje
    echo "Ya existe la carpeta"
else
    # Si no existe, crear la carpeta y mostrar un mensaje de error si falla
    if ! aws s3api put-object --bucket "$bucket" --key "batch/diario/$fecha_actual/" 2>> /home/srvbackup/errores/batch/"$nombre_archivo"; then
...
export LC_ALL=en_US.iso88591
    aws s3 sync "$directorio" "$ruta_s3""$directorio" --exclude "/batch/historicos/inv*" --exclude "/batch/historicos/histo*" --exclude "/batch/home/bctrl-m/ctm/*" --exclude "/batch/usr/cobol/salto/resumenes/*" --only-show-errors 2>> /home/srvbackup/errores/batch/"$nombre_archivo"
}
...
warnings=$(grep -c "warning" /home/srvbackup/errores/batch/"$nombre_archivo")
failed=$(grep -c "failed" /home/srvbackup/errores/batch/"$nombre_archivo")
...
# Generar un nuevo archivo con la fecha y hora actual para el segundo intento
    nombre_archivo=$(date +"%d-%m-%Y_%H-%M-%S").txt
    echo "-------- $(date +"%d-%m-%Y %H:%M:%S") ----------" > /home/srvbackup/errores/batch/"$nombre_archivo"
