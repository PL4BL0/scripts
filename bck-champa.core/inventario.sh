...
echo "$fecha_inicial" > /home/srvbackup/errores/inventario/"$nombre_archivo"
...
directorios_local=("/batch/disk4/$1")

# Nombre del bucket S3
bucket="backup-core-nx"

# Ruta en S3 donde quieres sincronizar los archivos
ruta_s3="s3://$bucket/inventario/$fecha_actual"

# Verificar si la carpeta ya existe en S3
if aws s3api head-object --bucket "$bucket" --key "inventario/$fecha_actual/" 2>/dev/null; then
    # Si existe, mostrar un mensaje
    echo "Ya existe la carpeta"
else
    # Si no existe, crear la carpeta y mostrar un mensaje de error si falla
    if ! aws s3api put-object --bucket "$bucket" --key "inventario/$fecha_actual/" 2>> /home/srvbackup/errores/inventario/"$nombre_archivo"; then
...
sincronizar_directorio() {
    local directorio="$1"
    aws s3 sync "$directorio" "$ruta_s3""$directorio"  --only-show-errors 2>> /home/srvbackup/errores/inventario/"$nombre_archivo"
}
...
warnings=$(grep -c "warning" /home/srvbackup/errores/inventario/"$nombre_archivo")
failed=$(grep -c "failed" /home/srvbackup/errores/inventario/"$nombre_archivo")
...
# Generar un nuevo archivo con la fecha y hora actual para el segundo intento
    nombre_archivo=$(date +"%d-%m-%Y_%H-%M-%S").txt
    echo "-------- $(date +"%d-%m-%Y %H:%M:%S") ----------" > /home/srvbackup/errores/inventario/"$nombre_archivo"
