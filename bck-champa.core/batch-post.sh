...
directorios_local=("/batch/disk4" "/batch/home" "/batch/tn" "/batch/tn/galicia" "/batch/disk2" "/batch/conversion" "/batch/dbcore"
 "/batch/procesos" "/batch/usr/cobol" "/batch/usr/cobol/pdr" "/batch/usr/cobol/salto/liq"
...
ruta_s3="s3://$bucket/batch/post/$fecha_actual"

# Verificar si la carpeta ya existe en S3
if aws s3api head-object --bucket "$bucket" --key "batch/post/$fecha_actual/" 2>/dev/null; then
    # Si existe, mostrar un mensaje
    echo "Ya existe la carpeta"
else
    # Si no existe, crear la carpeta y mostrar un mensaje de error si falla
    if ! aws s3api put-object --bucket "$bucket" --key "batch/post/$fecha_actual/" 2>> /home/srvbackup/errores/batch/"$nombre_archivo"; then
...
export LC_ALL=en_US.iso88591
    aws s3 sync "$directorio" "$ruta_s3""$directorio" --exclude "/batch/home/bctrl-m/*" --exclude "/batch/usr/cobol/salto/resumenes/*" --exclude "/batch/historicos/*" --only-show-errors 2>> /home/srvbackup/errores/batch/"$nombre_archivo"
}
